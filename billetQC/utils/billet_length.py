...
def lengthCalc(
    ...
    if(force_diagonal == False):
                    ...
                    if (data.shape[1] == 5):
                        ...
                        for i in range(data.shape[0]):
                            if((data[i][1] - data[i][3]/2.0) > 0.01) and ((data[i][1] + data[i][3]/2.0) < 0.99) and ((data[i][2] - data[i][4]/2.0) > 0.01) and ((data[i][2] + data[i][4]/2.0) < 0.99):
                                imgDetections = np.append(imgDetections, [[data[i][1]], [data[i][2]], [data[i][3]], [data[i][4]]], axis=1)
                                
                                if (data[i][4]*filter_ratio < data[i][3] ) or (data[i][3]*filter_ratio < data[i][4]):
                                    maxRatio = np.append(maxRatio, [[data[i][3]], [data[i][4]]])
                        print(imgDetections)
                        print(imgDetections[1,:])
                        if (imgDetections.size != 0):
                            if ((np.max(imgDetections[1,:]) > 0.73) or (np.min(imgDetections[1,:]) < 0.27)):
                                diff = np.max([np.max(imgDetections[1,:]) - 0.73, 0.27 - np.min(imgDetections[1,:])])
                               #print(diff)

                                load_height = np.append(load_height, (h*(((diff)/2)/(h+(diff+0.5)/2))))
                                #print(load_height)
                        allDetections = np.append(allDetections, imgDetections, axis=1)
                        imgDetections = np.array([[],[],[],[]])
                    ...
                    if (data.shape == 5):
                        ...
                        for i in range(data.shape[0]):
                            if((data[1] - data[3]/2.0) > 0.01) and ((data[1] + data[3]/2.0) < 0.99) and ((data[2] - data[4]/2.0) > 0.01) and ((data[2] + data[4]/2.0) < 0.99):
                                imgDetections = np.append(imgDetections, [[data[1]], [data[2]], [data[3]], [data[4]]], axis=1)
        ...
        if load_width == 0:
            scale = 1
        for i in range(int(allDetections.size/4)):

            allDetections[2][i] = allDetections[2][i]*scale
            allDetections[3][i] = allDetections[3][i]*scale
        ...
        for i in range(len(allDetections)):
            ...
            if allDetections[2][i] > allDetections[3][i]:
                l = allDetections[2][i]
                h = allDetections[3][i]
                #print(" l > h")
            else:
        ...
        for f in file_names:
            ...
            if(data.shape[0] != 0):
                if(data.ndim == 2):
                    if (data.shape[1] == 5):
                        #print('no confidence values associated with labels')
                        if ((np.max(data[:,2]) > 0.73) or (np.min(data[:,2]) < 0.27)):
                            diff = np.max([np.max(data[:,2]) - 0.73, 0.27 - np.min(data[:,2])])
                            load_height = np.append(load_height, (h*(((diff)/2)/(h+(diff+0.5)/2))))
                            
                        for i in range(data.shape[0]):
                            lengthGraphOut = np.append(lengthGraphOut, (data[i][3]**2 + data[i][4]**2)**0.5)
                    ...
                    if (data.shape == 5):
                        #print('no confidence values associated with labels')
                        if ((data[2] > 0.73) or (data[2] < 0.27)):
                            diff = np.max([np.max(data[2]) - 0.73, 0.27 - np.min(data[2])])
                            load_height = np.append(load_height, (h*(((diff)/2)/(h+(diff+0.5)/2))))
                        lengthGraphOut = np.append(lengthGraphOut, (data[3]**2 + data[4]**2)**0.5)
        ...
        if(load_width != 0):
            for i in range(len(lengthGraphOut)):
                lengthGraphOut[i] = lengthGraphOut[i]*scale
    ...
    if(save_img):
        ...
        s = "St. Dev: " + str(round(np.std(lengthGraphOut), 4))
        plt.text( np.average(lengthGraphOut) + 4, 1.5/np.average(lengthGraphOut) - 0.02, s)
        plt.title("Estimated Billet Lengths")
...
parser.add_argument('--length_multiplier', type=float, default=1)
parser.add_argument('--load_width', type=float, default=0.75)
parser.add_argument('--filter_ratio', type=float, default=4)
parser.add_argument('--keep_width', type=float, default=0.5)
