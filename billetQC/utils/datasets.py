...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        global cap # create this to stop the webcam from running when stop it
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        ...
        for i, s in enumerate(sources):
            ...
            # Change frame per second here, note that line 315 n = 4
    ...
    # create this to stop the webcam from running when stop it
    def stop(self): 
        if cap.isOpened():
            cap.release() 
            cv2.destroyAllWindows()
